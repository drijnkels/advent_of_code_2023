...
type HandTypes = {
  [key in 'five' | 'four' | 'fullhouse' | 'three' | 'two' | 'one' | 'high']: string[]
}

const handTypes: HandTypes = {
  five: [],
  four: [],
  fullhouse: [],
  three: [],
...
type CardValueTypes = {
  [key in 'A'| 'K' | 'Q' | 'J' | 'T']: number
}
// Translate letter cards to a value
const cardValues: CardValueTypes = {
  A: 14,
  K: 13,
  Q: 12,
  J: 11,
...
const getCardValues = (hand: string) => {
  const cardsInHand = hand.split('');
  const values = [];
  for (let card of cardsInHand) {
    if (isNaN(parseInt(card))) {
      values.push(cardValues[card as keyof CardValueTypes])
    }else{
